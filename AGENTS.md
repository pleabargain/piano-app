...
9. **Yes/No Question Format**: When asking simple yes/no confirmation questions, always append "Y/N" on its own line. Example: "Should I update the verify-dates test so it expects 2026-02-01? Y/N" — NOT "Should I update the verify-dates test so it expects 2026-02-01?" without the Y/N prompt.
10. **Binary Question Format**: When presenting binary choices or options to the user, always format them using A/B labels:
...
11. **Code Change Documentation**: All code changes must be dated with ISO timestamp format (YYYY-MM-DD) and tracked in README.md:
...
12. **Multiple Choice Question Format**: When presenting multiple choice options to the user, always format them using lowercase letter labels:
...
13. **Exercise URL Documentation**: If an exercise has a URL (accessible at `http://localhost:5173/exercise/{exercise-id}`), that exercise MUST be documented and visible in `http://localhost:5173/usage-ideas.html`:
...
14. **URL Lesson Visibility**: If you create a new URL-based lesson or exercise, the link to this lesson MUST be added to `http://localhost:5173/usage-ideas.html` (via `usage-ideas.md`) so users can discover and access it easily.
