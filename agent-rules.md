...
1. **Check for Latest Library Versions**: At the beginning of a project or when starting work, verify the project uses the latest stable dependency versions. Use `npm outdated` or `npm view <package> version` to check for updates. Update dependencies to their latest stable versions unless there are specific compatibility reasons not to. If you cannot update, document the reason in README.md.
2. **Create Limit and Regression Tests**: Always create unit tests that cover boundary/limit behavior and regression tests so no functionality is lost when refactoring or adding features.
3. **Granular Testing**: If a test fails, stop rerunning it repeatedly. Instead, create new, more granular tests for each specific function or component connected to the failing test to isolate the issue.
4. **File Save UI**: When a user wants to save a file, the application must show a UI dialog that allows the user to navigate the current root directory (or file system) to choose where to save the file. The default filename must use ISO timestamp format: `YYYY-MM-DD-HH-MM-SS` (example: `2026-01-09-14-30-45`).
5. **Best Practices for Debugging User Interfaces**:
   ...
   - **Test Error Paths Explicitly**: Do not just test happy paths. Write tests for:
     - Invalid input data
     - API failures
     - Network errors
     - Permission denials
   ...
   - **Check API Availability**: Before using browser APIs, verify they exist and are callable (for example, `typeof window !== 'undefined' && 'showSaveFilePicker' in window && typeof window.showSaveFilePicker === 'function'`).
   - **Test Fallback Mechanisms**: If a primary method fails, ensure fallback mechanisms work correctly and are tested.
   - **Component State Management**: When debugging React components, verify that state updates correctly and that errors do not leave components in inconsistent states.
...
7. **Chrome DevTools MCP Server for Testing**: Use the Chrome DevTools MCP server to help plan unit tests and debug UI behavior. Leverage browser automation tools to:
...
8. **High Contrast UI Text**: Do NOT use low-contrast or "low definition" font colors. All UI text must be high contrast and readable against its background (including disabled states, placeholders, helper text, and secondary labels).
...
10. **Code Change Documentation**: All code changes must be dated with ISO timestamp format (`YYYY-MM-DD`) and tracked in README.md:
   - Add ISO timestamp comments in code files when making significant changes (example: `// 2026-01-11: Enhanced scale progression UI`)
   ...
   - Ensure README.md ends with a dated "Last updated: YYYY-MM-DD" line
